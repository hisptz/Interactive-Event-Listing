  ...
  getOrgunitLevelsInformation() {
    return Observable.create(observer => {
      if (this.orgUnitLevels.length !== 0) {
        ...
        this.http.get('organisationUnitLevels.json?fields=id,name,level&order=level:asc').subscribe(
          (levels: any[]) => {
  ...
  getOrgunitGroups() {
    return Observable.create(observer => {
      if (this.orgUnitGroups.length !== 0) {
        ...
        this.http.get('organisationUnitGroups.json?fields=id,name&paging=false').subscribe(
          (groups: any) => {
  ...
  getAllOrgunitsForTree1(fields = null, orgunits = null) {
    return Observable.create(observer => {
      if (this.nodes !== null) {
        ...
        this.http
          .get('organisationUnits.json?fields=' + fields + '&filter=id:in:[' + orgunits.join(',') + ']&paging=false')
          .subscribe(
            (nodes: any) => {
